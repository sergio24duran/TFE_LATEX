...
\tableofcontents
	...
	\end{equation}\smallskip
	 ...
	 A continuación veremos la forma canónica mas común para los sistemas bizonales que coloca la recta separación en el ejde de ordenadas $x_1=0$.
	...
	\begin{equation}
		B_1-B_2=(B_1-B_2)e_1e_1^T
	\end{equation}
	Con $e_1=(1,0)^T$ el primer vector de la base canónica.
    \end{definicion}
	\newpage
	
	\textbf{Demostración}: Basta con realizar una simetría par que el vector $w$, normal a la recta de separación $x^T\cdotp w+\delta=0$, se transforme en el vector $e_1=(1,0)^T$, se puede hacer usando matrices de Householder. Por último solo queda por hacer una translación parq que la nueva recta de separación (ahora vertical) pase por el origen. Ahora el sistema tiene la forma:
	...
	Como el $(0,0)^T$ está en la recta de separación $x_1=0$, sustituyendo en \eref{eq:b1b2} se tiene que $c_1=c_2$ por lo que:
	...
	\begin{equation}
		\label{eq:cambio}
		y_2=-b_{12}\,x_2-c_1\quad \rightarrow \quad x_2=\frac{-y_2-c_1}{b_{12}}
	\end{equation}\smallskip
	\newpage
	...
	\begin{equation}
		\label{eq:q1}
	\begin{aligned}
		\dot{x}_1&=b_{11}^1x_1+b_{12}x_2+c-1=b_{11}^1x_1+b_{12}\left(\frac{-y_2-c_1}{b_{12}}\right)+c_1=b_{11}^1x_1-y_2 \nonumber \\[2mm]
		\dot{y}_2&=-b_{12}\dot{x}_2=-b_{12}\left(b_{21}^1x_1+b_{22}x_2+c_2\right)\nonumber \\[2mm]
		&=-b_{12}b_{21}^1x_1+b_{22}\left(-b_{12}x_2\right)-b_{12}c_2 \nonumber \\[2mm]
		&=-b_{12}b_{21}^1x_1+b_{22}\left(y_2+c_1\right)-b_{12}c_2 \nonumber \\[2mm]
		&=-b_{12}b_{21}^1x_1+b_{22}y_2+b_{22}c_1-b_{12}c_2
	\end{aligned}
	\end{equation}\smallskip
	
	De forma análoga para $x_1>0$:
	\begin{equation}
		\label{eq:q2}
		\scalebox{1.2}{$\displaystyle
			\left\{
			\begin{aligned}
			&\dot{x}_1=b_{11}^2x_1-y_2 \\[2mm]
			&\dot{y}_2=-b_{12}b_{21}^2x_1+b_{22}y_2+b_{22}c_1-b_{12}c_2
		\end{aligned}
		\right.
		$}
	\end{equation}\smallskip
	
	Por tanto aplicando lo anterior al sistema \eref{eq:lie-1} y tomando \\ $c_{11}^i=b_{11}^i \: ; \: c_{121}^i=-b_{12}b_{21}^i \: ; \: c_{22}=b_{22} \: ; \: d_2=b_{22}$
	 
	
	
	\begin{theorem}\label{t2}
		Cuando las primeras componentes de las segundas columnas de las matrices $B_1$ y $B_2$ no son nulas, existe un cambio de variable que transforma el sistema \eref{eq:biz1} en la forma canónica de Liénard, sin modificar la traza y el determinanate de la matriz característica ya que son invariates algebraicos:
	\end{theorem}
	...
	\begin{equation}
		...
		\scalebox{1}{$\displaystyle
			...
			\begin{aligned}
				...
				\end{pmatrix} \quad si \quad x_1 > 0
			\end{aligned}
			\right. \qquad\quad a \in \left\{-1,0,1\right\}
			$}
	\end{equation}\smallskip
	
	El requerimiento sobre las matrices del Teorema \ref{t2} implica la observabilidad del sistema desde el punto de vista de teoría de control
